...
const BuyNumber = () => {
  ...
  const handleServerClick = async (server) => {
    ...
    try {
      ...
      // const selectedServer=selectedServerjson.apiEndpoint;
  ...
  useEffect(() => {
    ...
    const fetchServices = async () => {
      try {
        const response = await fetch('http://localhost:8081/services1'); // Update the URL accordingly
  ...
  const getNumber = async(serverEndpoint,selectedService) => {
    ...
    try {
      ...
      const phoneNumber = data.split(':')[2];
  
      // Update the state with the obtained phone number
      setPhoneNumber(phoneNumber);
  
      // Placeholder for the actual message from the API response
